...
| brand | The brand of device.For example:OPPO,XIAOMI,HUAWEI,APPLE    |
...
| Macro         | Description| If necessary |
|---------------|------------------------------------------------------------------------------------------------------------------------------|--------------|
| {chn}         | Your sub_source_id or placement_id | No |
| {oid}        | Offer ID| No |
| {package}     | The package name of this offer | No |
| {payout}     | The settled price (in USD) for this conversion. | Yes |
| {aff_sub}    | Your click_id or transaction_id. Ad network unique click identifier | Yes |
| {aff_sub2}    | Passed in tracking link | No |
| {aff_sub3}    | Passed in tracking link | No |
| {advid}    | Google Advertising ID | No |
| {idfa}    | IOS Identifier for Advertisers | No |
| {clk_ip}    | Callback user's click IP | No |
| {order_id}    | Conversion ID, a unique identifier for each conversion generated by Youmi. The same order_id means the same conversion. Developers should note that the same order_id can only be settled once | No |
